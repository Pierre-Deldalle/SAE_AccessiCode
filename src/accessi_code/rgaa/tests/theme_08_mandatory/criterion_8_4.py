    ...
    async def run(
        ...
        if analysis.confidence == "low":
            return TestResult(
                ...
                self.criterion_id,
                TestStatus.NEEDS_REVIEW,
                (analysis.explanation or ("L'analyse automatique de la langue n'est pas suffisamment fiable.")),
                findings=[
                    Finding(
                        element="html",
                        message=("L'analyse IA possède un niveau de confiance insuffisant."),
                        recommendation=("Vérifier manuellement que la langue déclarée correspond au contenu."),
                        evidence=evidence,
                    )
                ],
                tested_elements=1,
